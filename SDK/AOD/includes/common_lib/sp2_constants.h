...
namespace SP2
{
   ...
   const REAL32 c_fApprovalRatioRaisePersonalTax = 1.75f;
	...
	//1.5% for AOD, 75% is way too much
	const REAL32 c_fPourcPopulationAbleToEnroll = 0.015f;
	...
	const REAL32 c_pGvtTypeGvtSpendingsModifier[EGovernmentType::ItemCount] = 
   {
	...
	const REAL64 c_fTelecomIterationPeriod = 73000.f; // 4 years, 200 years for AOD
	const REAL64 c_fInfrastrucutureIterationPeriod = 73000.f; // 4 years, 200 years for AOD
	const REAL64 c_fBirthRateIterationPeriod = 365.f; // 4 years, 1 year for AOD
	const REAL64 c_fDeathRateIterationPeriod = 365.f; // 4 years, 1 year for AOD
...
#define DEBUG_VALIDATE_COORDINATE(in_Vector2D)\
   ...
   //HDM save file markers
   static const UINT32 c_iSdkModGameType = MAKEFOURCC('H', 'D', 'M', '\0');
   static const INT32 c_iSdkModMajorRelease = 10;
   static const INT32 c_iSdkModMinorRelease = 9;
   static const INT32 c_iSdkModPatchRelease = 1;

   //SDK mod name to display in main menu and server list
   static const GString c_sSdkModName = L"Human Dev Mod " + GString(c_iSdkModMajorRelease) + L"." + GString(c_iSdkModMinorRelease) + (c_iSdkModPatchRelease > 0 ? (L"." + GString(c_iSdkModPatchRelease)) : L"");

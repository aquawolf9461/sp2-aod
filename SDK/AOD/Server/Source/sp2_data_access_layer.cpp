      ...
      {         
        ...
        if(l_bUseNewExportMechanics)
        {
            ...
            for(auto l_It = l_mExportRank.begin(); l_It != l_mExportRank.end(); ++l_It)
            {
                ...
                const REAL64 l_fExportStrength = l_fGdpPerCapita * max(l_fGdpPercent, 0.8f * l_fMarketShare);
            ...
            //Non-service balance ratios
            {
                ...
                GDZLOG(EDZLogLevel::Info2, l_pCountryData->NameAndIDForLog() + L" non-service balance ratio = " + GString(l_fNonServiceBalanceRatio));
      ...
      if(l_bUseNewExportMechanics)
      {
          for(auto l_ResourceIt = l_mExportRank.cbegin(); l_ResourceIt != l_mExportRank.cend(); ++l_ResourceIt)
          {
              ...
              for(auto l_CountryIt = l_mExportRankForResource.cbegin(); l_CountryIt != l_mExportRankForResource.cend(); ++l_CountryIt)
              {
                  ...
                  GDZLOG(EDZLogLevel::Info2, CountryData(l_CountryIt->second)->NameAndIDForLog() + L" " + g_ServerDAL.GetString(g_ServerDAL.StringIdResource(l_eResource)) + L" export rank = " + GString(l_iRank));
                  l_iRank--;
              }
          }
      }
       ...
       //Make sure that no covert action cell is found in a deactivated country
       {
           ...
           else
           {
               ...
               for(UINT32 i = 1 ; i <= l_iNbCountry ; i++)
               {
                   ...
                   {
                       gassert(l_vCells[j].OwnerID() == i,L"Cell owner mismatch " + GString(l_vCells[j].OwnerID()) + L" vs " + GString(i));
															...
															UINT32 in_iPlayerModId)
{
                                        ...
                                        l_It++)
   {
      ...
      const ENTITY_ID l_iRegionOwner = m_vRegionControl[l_pGroup->RegionToBombard()].m_iPolitical;
      gassert(DiplomaticStatus(l_iGroupOwner, l_iRegionOwner) == EDiplomaticStatus::Hostile,L"Peacetime bombardment");
...
bool GDataAccessLayerServer::OnLoad(GOBuffer& io_Buffer)
{
   ...
   // Can't use country names or NameAndIDForLog yet, as country names haven't been loaded
   ...
   // Load units
   {
      ...
      UINT32 l_iCount;
      io_Buffer >> l_iCount;
      GDZLOG(EDZLogLevel::Info1, L"Unit group count = " + GString(l_iCount));
      ...
      for(UINT32 i = 0;i < l_iCount;i ++)
	   {
         ...
         const UINT32 l_iId = l_pGroup->Id();
         const ENTITY_ID l_iOwnerId = l_pGroup->OwnerId();
         const EMilitaryStatus::Enum l_eStatus = l_pGroup->Status();
         GDZLOG(EDZLogLevel::Info2, L"Unit group " + GString(i) + L", ID " + GString(l_iId) + L" of " + GString(l_iOwnerId) + L", location " + GString(l_pGroup->Position().x) + L", " + GString(l_pGroup->Position().y) + L", status " + GString(l_eStatus));
         IF_RETURN_LOG(l_iOwnerId > NbCountry(), false, "Loaded military unit owner has no matching game country");
         ...
         io_Buffer >> l_pGroup->m_iRegionID
                   ...
                   >> l_pGroup->m_iPrevMoveRegionID
                   >> l_pGroup->m_fLastMoveValue;
         GDZLOG(EDZLogLevel::Info2, L"In region " + RegionNameAndIDForLog(l_pGroup->m_iRegionID) + L", destination action " + GString(l_pGroup->m_ActionAtDest) + L", move region " + RegionNameAndIDForLog(l_pGroup->m_iMoveRegionID) + L", previous move region " + RegionNameAndIDForLog(l_pGroup->m_iPrevMoveRegionID) + L", last move value " + GString(l_pGroup->m_fLastMoveValue));

         if((l_eStatus == EMilitaryStatus::Moving) && (l_pGroup->m_iMoveRegionID == 0))
         {
             GDZLOG(EDZLogLevel::Warning, L"Unit ID " + GString(l_iId) + L" of " + GString(l_iOwnerId) + L" is moving but has no destination; changing to ready");
         ...
         GDZLOG(EDZLogLevel::Info2, L"Unit count = " + GString(l_iUnitCount));
         ...
         for(UINT32 j = 0;j < l_iUnitCount;j ++)
		   {
            ...
            {
               GDZLOG(EDZLogLevel::Info2, L"Fight info present for unit " + GString(j));
            ...
            GDZLOG(EDZLogLevel::Info2, L"Unit " + GString(j) + L" l_iDesignID = " + GString(l_iDesignID));
            l_pUnit->Design(g_Joshua.UnitManager().UnitDesign(l_iDesignID) );

            g_Joshua.UnitManager().AddNewUnit(l_pUnit);

            // Add loaded unit to group
            // Group will become naval when first naval unit is added
            g_Joshua.UnitManager().AddUnit2Group(l_pUnit, l_pGroup);
		   }

         GDZLOG(EDZLogLevel::Info2, L"Naval: " + GString(l_pGroup->IsNaval()));
         if(l_pGroup->IsNaval() && l_pGroup->m_iRegionID != 0)
         {
             GDZLOG(EDZLogLevel::Warning, L"Naval unit ID " + GString(l_iId) + L" of " + GString(l_iOwnerId) + L" is out of water in " + RegionNameAndIDForLog(l_pGroup->m_iRegionID) + L"; relocating onto nearby water");

             l_pGroup->SetConsistentPosition();
         }
   ...
   // Load cities
   {
      UINT32 l_iCount;
      io_Buffer >> l_iCount;
      GDZLOG(EDZLogLevel::Info1, L"City count = " + GString(l_iCount));
...
{
    return ((in_iRegionID == 0) ? "" : (GetString(StringIdRegion(in_iRegionID)) + L" ")) + L"(" + GString(in_iRegionID) + L")";
}
...
void GDataAccessLayerServer::SaveHdmData(GIBuffer& io_Buffer)
{
    GDZLOG(EDZLogLevel::Entry, L"io_Buffer.Size() = " + GString(io_Buffer.Size()));

    do
    {
        if(!g_SP2Server->CompatibleSaveFormat())
        {
        ...
        //Save markers
        io_Buffer << c_iSdkModGameType
                  << c_iSdkModMajorRelease
                  << c_iSdkModMinorRelease
                  << c_iSdkModPatchRelease;

        //10.9 HDI components and name
        for(ENTITY_ID i = 1; i <= NbCountry(); i++)
        {
            io_Buffer << m_pCountryData[i].LifeExpectancy()
                      << m_pCountryData[i].MeanYearsSchooling()
                      << m_pCountryData[i].ExpectedYearsSchooling()
                      << m_pCountryData[i].Name();
        }
    } while(false);
...
void GDataAccessLayerServer::LoadHdmData(GOBuffer& io_Buffer)
{
    GDZLOG(EDZLogLevel::Entry, L"io_Buffer.Remaining() = " + GString(io_Buffer.Remaining()));
    ...
    do
    {
        ...
        {
            g_Joshua.Log(L"Save file doesn't contain HDM markers, treating as non-HDM save", MSGTYPE_MSG);
            break;
        }
        
        UINT32 l_iSdkModGameType = 0;
        INT32 l_iSdkModMajorRelease = 0;
        INT32 l_iSdkModMinorRelease = 0;
        INT32 l_iSdkModPatchRelease = 0;

        io_Buffer >> l_iSdkModGameType
                  >> l_iSdkModMajorRelease
                  >> l_iSdkModMinorRelease
                  >> l_iSdkModPatchRelease;

        if(l_iSdkModGameType != c_iSdkModGameType)
        {
            const GString l_sNonHdmSdkModGameType = L"Save file doesn't contain valid HDM markers, treating as non-HDM save";
            GDZLOG(EDZLogLevel::Always, l_sNonHdmSdkModGameType + L"; expected " + GDZDebug::FormatHex(c_iSdkModGameType) + L", got " + GDZDebug::FormatHex(l_iSdkModGameType));
            g_Joshua.Log(l_sNonHdmSdkModGameType, MSGTYPE_MSG);
            break;
        }

        g_Joshua.Log(L"HDM save file " + GString(l_iSdkModMajorRelease) + L"." + GString(l_iSdkModMinorRelease) + (l_iSdkModPatchRelease > 0 ? (L"." + GString(l_iSdkModPatchRelease)) : L""), MSGTYPE_MSG);

        //10.9 HDI components and name
        if((l_iSdkModMajorRelease == 10 && l_iSdkModMinorRelease >= 9) || l_iSdkModMajorRelease > 10)
        {
            for(ENTITY_ID i = 1; i <= NbCountry(); i++)
            {
                ...
                m_pCountryData[i].LifeExpectancy(l_fLifeExpectancy);
                m_pCountryData[i].MeanYearsSchooling(l_fMeanYearsSchooling);
                m_pCountryData[i].ExpectedYearsSchooling(l_fExpectedYearsSchooling);
                m_pCountryData[i].Name(l_sName);

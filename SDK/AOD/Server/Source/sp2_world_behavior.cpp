...
{	
	...
	l_iNew1565 = ReturnInteger64((1.0f/15.0f) * m_fFrequency * (REAL32)(in_pRegion->Population15() - l_iDeaths15));
	...
	// 1/1000 for AOD
	l_iNew65 = ReturnInteger64((1.0f/1000.f) * m_fFrequency * (in_pRegion->Population1565() - l_iDeaths1565));
	...
	*/
		...
		l_Itr++)
	{
		if(l_iTemp != 0)
		{
            ...
            l_iTempCalc = max(l_iTempCalc, -l_Itr->second);
	...
	//If region had no religions before, then check the most popular religion for that country.
	if(l_Religions.empty() && (l_iTemp != 0 || l_iTempLeftToArrive != 0 || l_iTempLeftToLeave != 0))
	{
		INT64 l_iMaxPopulationReligion = -1;
        INT32 l_iMaxReligion = -1;
        ...
        in_pRegion->InsertReligion(l_iMaxReligion, 0);
	}

    if(l_iTemp != 0 || l_iTempLeftToArrive != 0 || l_iTempLeftToLeave != 0)
    {
        in_pRegion->GetReligions(l_Religions);
        multimap<INT64, INT32, greater<INT64>> l_mSortedReligions;
        for(auto l_Itr = l_Religions.cbegin(); l_Itr != l_Religions.cend(); ++l_Itr)
            l_mSortedReligions.insert(pair<INT64, INT32>(l_Itr->second, l_Itr->first));

        if(l_iTempLeftToArrive != 0)
        {
            //we have not put every new people in, so we add the remaining at the largest religion population
            const INT32 l_iLargestReligion = l_mSortedReligions.cbegin()->second;
            in_pRegion->ReligionChangePopulation(l_iLargestReligion, in_pRegion->ReligionGetPopulation(l_iLargestReligion) + l_iTempLeftToArrive);
            l_iPopulationReligion += l_iTempLeftToArrive;
        }

        for(auto l_Itr = l_mSortedReligions.cbegin(); l_Itr != l_mSortedReligions.cend() && (l_iTemp != 0 || l_iTempLeftToArrive != 0 || l_iTempLeftToLeave != 0); ++l_Itr)
        {
            if(l_iTemp != 0)
            {
                //we have not put every new people in, so we add the remaining at the largest religion population
                const INT64 l_iReligionPop = in_pRegion->ReligionGetPopulation(l_Itr->second);
                const INT64 l_iTempCalc = max(l_iTemp, -l_iReligionPop);
                in_pRegion->ReligionChangePopulation(l_Itr->second, l_iReligionPop + l_iTempCalc);
                l_iPopulationReligion += l_iTempCalc;
                l_iTemp -= l_iTempCalc;
            }
            if(l_iTempLeftToLeave != 0)
            {
                //we have not put every new people in, so we add the remaining at the largest religion population
                const INT64 l_iReligionPop = in_pRegion->ReligionGetPopulation(l_Itr->second);
                const INT64 l_iTempCalc = min(l_iTempLeftToLeave, l_iReligionPop);
                in_pRegion->ReligionChangePopulation(l_Itr->second, l_iReligionPop - l_iTempCalc);
                l_iPopulationReligion -= l_iTempCalc;
                l_iTempLeftToLeave -= l_iTempCalc;
            }
	...
	//If region had no languages before, then check the most popular language for that country.
	if(l_Languages.empty() && (l_iTemp != 0 || l_iTempLeftToArrive != 0 || l_iTempLeftToLeave != 0))
	{
		INT64 l_iMaxPopulationLanguage = -1;
        INT32 l_iMaxLanguage = -1;
        ...
        in_pRegion->InsertLanguage(l_iMaxLanguage, 0);
	}

    if(l_iTemp != 0 || l_iTempLeftToArrive != 0 || l_iTempLeftToLeave != 0)
    {
        in_pRegion->GetLanguages(l_Languages);
        multimap<INT64, INT32, greater<INT64>> l_mSortedLanguages;
        for(auto l_Itr = l_Languages.cbegin(); l_Itr != l_Languages.cend(); ++l_Itr)
            l_mSortedLanguages.insert(pair<INT64, INT32>(l_Itr->second, l_Itr->first));

        if(l_iTempLeftToArrive != 0)
        {
            //we have not put every new people in, so we add the remaining at the largest Language population
            const INT32 l_iLargestLanguage = l_mSortedLanguages.cbegin()->second;
            in_pRegion->LanguageChangePopulation(l_iLargestLanguage, in_pRegion->LanguageGetPopulation(l_iLargestLanguage) + l_iTempLeftToArrive);
            l_iPopulationLanguage += l_iTempLeftToArrive;
        }

        for(auto l_Itr = l_mSortedLanguages.cbegin(); l_Itr != l_mSortedLanguages.cend() && (l_iTemp != 0 || l_iTempLeftToArrive != 0 || l_iTempLeftToLeave != 0); ++l_Itr)
        {
            if(l_iTemp != 0)
            {
                //we have not put every new people in, so we add the remaining at the largest Language population
                const INT64 l_iLanguagePop = in_pRegion->LanguageGetPopulation(l_Itr->second);
                const INT64 l_iTempCalc = max(l_iTemp, -l_iLanguagePop);
                in_pRegion->LanguageChangePopulation(l_Itr->second, l_iLanguagePop + l_iTemp);
                l_iPopulationLanguage += l_iTempCalc;
                l_iTemp -= l_iTempCalc;
            }
            if(l_iTempLeftToLeave != 0)
            {
                //we have not put every new people in, so we add the remaining at the largest Language population
                const INT64 l_iLanguagePop = in_pRegion->LanguageGetPopulation(l_Itr->second);
                const INT64 l_iTempCalc = min(l_iTempLeftToLeave, l_iLanguagePop);
                in_pRegion->LanguageChangePopulation(l_Itr->second, l_iLanguagePop - l_iTempCalc);
                l_iPopulationLanguage -= l_iTempCalc;
                l_iTempLeftToLeave -= l_iTempCalc;
            }
        }
...
bool GWorldBehavior::Iterate_Demand(GRegion* in_pRegion)
	...
	REAL64 l_fInflationGrowth = (REAL64)m_CountryData->InflationLevelExpected() + 1.f;
	...
	l_fTempNewDemand = (in_pRegion->ResourceDemand(EResources::Cereals) 
		* l_fGDPGrowth * l_fInflationGrowth) 
		+ (0.05f * in_pRegion->ResourceDemand(EResources::Meat))
		+ (0.01f * in_pRegion->ResourceDemand(EResources::Dairy));
	l_NewDemand[EResources::Cereals] = ((l_fTempNewDemand 
		- in_pRegion->ResourceDemand(EResources::Cereals)) * l_fFrequency) 
		+ in_pRegion->ResourceDemand(EResources::Cereals);

	//Vegetables
	l_fTempNewDemand = (in_pRegion->ResourceDemand(EResources::Vegetable_Fruits) * l_fGDPGrowth * l_fInflationGrowth);
	...
	//Meat
	l_fTempNewDemand = (in_pRegion->ResourceDemand(EResources::Meat) * l_fGDPGrowth * l_fInflationGrowth);
	...
	//Dairy
	l_fTempNewDemand = (in_pRegion->ResourceDemand(EResources::Dairy) * l_fGDPGrowth * l_fInflationGrowth);
	...
	//Tobacco
	l_fTempNewDemand = (in_pRegion->ResourceDemand(EResources::Tobacco) * l_fGDPGrowth * l_fInflationGrowth);
	...
	//Drugs
	l_fTempNewDemand = (in_pRegion->ResourceDemand(EResources::Drugs) * l_fGDPGrowth * l_fInflationGrowth);
...
{
	const REAL32 c_fMaxBirthRate = 0.025f;
	const REAL32 c_fMinBirthRate = 0.015f;
	...
	REAL32 l_fHumanDevelopment = 2.3f - FindHumanDevelopmentFactor(m_CountryData->CountryID());
	REAL32 l_fGdpPop = 2.f - FindGDPPopulationFactor(m_CountryData->CountryID());
...
{
	const REAL32 c_fMaxDeathRate = 0.015f;
	const REAL32 c_fMinDeathRate = 0.01f;

	const REAL32 PourcHealthCare			= 0.3f;
	const REAL32 PourcHumanDevelopment	= 0.4f;
	const REAL32 PourcGdpPop				= 0.3f;
	//const REAL32 PourcStability = 0.4f;

	//REAL32 l_fGvtStability = 1.f - (m_CountryData->GvtStability());
	REAL32 l_fHealthCare = 2.f - (REAL32)m_CountryData->BudgetExpenseHealthcareRatio();
	REAL32 l_fHumanDevelopment = 1.2f - FindHumanDevelopmentFactor(m_CountryData->CountryID());
	REAL32 l_fGdpPop = 1.2f - FindGDPPopulationFactor(m_CountryData->CountryID());

	REAL32 l_fResults = (PourcHealthCare * l_fHealthCare) 
		+ (PourcHumanDevelopment * l_fHumanDevelopment)
		+ (PourcGdpPop * l_fGdpPop);
		//+ (PourcStability * l_fGvtStability);
...
void GWorldBehavior::Iterate_Emigration_Level_Expected()
{
	...
	const REAL32 HighestEmigration = 0.005f;
...
bool GWorldBehavior::Iterate_Pop_In_Poverty()
{
	...
	//0.1 of m_fFrequency for AOD, to slow it down
	l_fNewPoverty += (((l_fNewPoverty * l_fResults) - l_fNewPoverty) * (0.1f * m_fFrequency));	
...
{	
    ...
    if(l_iNewOver15Pop > 0)
    {
        ...
        m_CountryData->MeanYearsSchooling(0.f);
...
{
	const REAL32 MaxImmigrationLevel = 0.005f;
...
{
    GDZLOG(EDZLogLevel::Entry, L"in_iTreatyID = " + GString(in_iTreatyID) + L", in_bWorldMarket = " + GString(in_bWorldMarket));
		...
		i != l_EconomicRanks.end(); i++)
	{
		...
		for(UINT32 k = 0; k < EResources::ItemCount; k++)
		{
			...
			{		
				...
				if(l_iExporterId == l_iImporterId)
					continue;								
                ...
                //Limit service exports //Limit food exports for AOD
				const bool l_bServiceResource = l_iResource <= EResources::Drugs;

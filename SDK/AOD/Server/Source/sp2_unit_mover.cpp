...
//Changed for AOD, pre-industrial movement speeds
const REAL32 c_fWaterCostModifier = 50.0f;
const REAL32 c_fOwnedCostModifier = 100.0f;
const REAL32 c_fAllyCostModifier = 100.0f;
const REAL32 c_fEnemyCostModifier = 100.0f;
         ...
         if(!l_bImpossible)
         {
            ...
            if(l_pPath)
            {
              ...
              else
              {
                  ...
                  if(l_bOwnerIsHuman)
                      g_SP2Server->SendChatMessage(SDK::Event::ESpecialTargets::Server, l_pOwnerPlayer->Id(), L"Can't move to territory of " + l_pDestOwnerCountry->Name() + L"; not in an offensive war with that country", true);
              }
            }
            else
            {

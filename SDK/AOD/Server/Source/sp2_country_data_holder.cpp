...
void GCountryData::IterateDemand()
{
	...
	m_pResourceDemand[EResources::Meat] = (l_fTemp * (8.f * 498.2f)) + ((REAL64)m_iPopulation * 30.f);
	m_pResourceDemand[EResources::Vegetable_Fruits] = (l_fTemp * (8.f * 764.3f)) + ((REAL64)m_iPopulation * 40.f);
	m_pResourceDemand[EResources::Dairy] = (l_fTemp * (8.f * 830.9f)) + ((REAL64)m_iPopulation * 25.f);
	m_pResourceDemand[EResources::Tobacco] = (l_fTemp * (8.f * 143.49f)) + ((REAL64)m_iPopulation * 4.f);
	m_pResourceDemand[EResources::Drugs] = (l_fTemp * (8.f * 95.18f)) + ((REAL64)m_iPopulation * 5.f);
	m_pResourceDemand[EResources::Minerals] = (l_fTemp * 567.25f) + ((REAL64)m_iPopulation * 30.f);
	m_pResourceDemand[EResources::Fossile_Fuels] = (l_fTemp * 15420.7f) + ((REAL64)m_iPopulation * 40.f);
	...
	m_pResourceDemand[EResources::Construction] = (l_fTemp * (0.05f * 6473.3f)) + ((REAL64)m_iPopulation * 130.f);
	m_pResourceDemand[EResources::Health_Care] = (l_fTemp * (0.05f * 11158.28f)) + ((REAL64)m_iPopulation * 30.f);
	m_pResourceDemand[EResources::Retail] = (l_fTemp * (0.07f * 1456.18f)) + ((REAL64)m_iPopulation * 260.f);
	m_pResourceDemand[EResources::Legal_Services] = (l_fTemp * (0.1f * 2739.6f)) + ((REAL64)m_iPopulation * 25.f);

	m_pResourceDemand[EResources::Cereals] = (((l_fTemp * (8.f * 592.46f)) + ((REAL64)m_iPopulation * 75.f))*0.94f) 
		+ (0.01f * m_pResourceDemand[EResources::Dairy]) 
		+ (0.05f * m_pResourceDemand[EResources::Meat]);
	...
	m_pResourceDemand[EResources::Plastics] = (((l_fTemp * (2.f * 615.9f)) + ((REAL64)m_iPopulation * 10.f))*0.97f) 
		+ (0.01f * m_pResourceDemand[EResources::Luxury_Commodities]) 
		+ (0.02f * m_pResourceDemand[EResources::Commodities]);

	m_pResourceDemand[EResources::Fabrics] = (((l_fTemp * (6.22f * 1138.81f)) + ((REAL64)m_iPopulation * 75.f))*0.94f) 
		+ (0.01f * m_pResourceDemand[EResources::Commodities]) 
		+ (0.05f * m_pResourceDemand[EResources::Luxury_Commodities]);

	m_pResourceDemand[EResources::Pharmaceuticals] = (((l_fTemp * (6.61f * 1286.97f)) + ((REAL64)m_iPopulation * 5.f))*0.96f) 
		+ (0.04f * m_pResourceDemand[EResources::Health_Care]);

	m_pResourceDemand[EResources::Chemicals] = (((l_fTemp * (2.f * 1517.f)) + ((REAL64)m_iPopulation * 40.f))*0.95f) 
		+ (0.01f * m_pResourceDemand[EResources::Luxury_Commodities]) 
		+ (0.02f * m_pResourceDemand[EResources::Commodities]) 
		+ (0.01f * m_pResourceDemand[EResources::Plastics]) 
		+ (0.01f * m_pResourceDemand[EResources::Pharmaceuticals]);

	m_pResourceDemand[EResources::Machinery] = (((l_fTemp * 413.81f) + ((REAL64)m_iPopulation * 12.f))*0.84f) 
		+ (0.05f * m_pResourceDemand[EResources::Construction]) 
		+ (0.03f * m_pResourceDemand[EResources::Fossile_Fuels]) 
		+ (0.02f * m_pResourceDemand[EResources::Wood_Paper]) 
		+ (0.05f * m_pResourceDemand[EResources::Minerals]) 
		+ (0.01f * m_pResourceDemand[EResources::Precious_Stones]);	

	m_pResourceDemand[EResources::Electricity] = (((l_fTemp * 14010.75f) + ((REAL64)m_iPopulation * 35.f))*0.78f) 
		+ (0.02f * m_pResourceDemand[EResources::Wood_Paper]) 
		+ (0.04f * m_pResourceDemand[EResources::Plastics]) 
		+ (0.02f * m_pResourceDemand[EResources::Fabrics]) 
		+ (0.01f * m_pResourceDemand[EResources::Chemicals]) 
		+ (0.02f * m_pResourceDemand[EResources::Pharmaceuticals]) 
		+ (0.04f * m_pResourceDemand[EResources::Appliances]) 
		+ (0.02f * m_pResourceDemand[EResources::Vehicules]) 
		+ (0.02f * m_pResourceDemand[EResources::Machinery]) 
		+ (0.01f * m_pResourceDemand[EResources::Commodities]) 
	...
	m_pResourceDemand[EResources::Iron_Steel] = (((l_fTemp * 1078.88f) + ((REAL64)m_iPopulation * 50.f))*0.82f) 
		+ (0.04f * m_pResourceDemand[EResources::Appliances]) 
		+ (0.04f * m_pResourceDemand[EResources::Vehicules]) 
		+ (0.04f * m_pResourceDemand[EResources::Machinery]) 
		+ (0.06f * m_pResourceDemand[EResources::Construction]);	

	m_pResourceDemand[EResources::Engineering] = (((l_fTemp * (0.1f * 2687.87f)) + ((REAL64)m_iPopulation * 20.f))*0.98f) 
		+ (0.02f * m_pResourceDemand[EResources::Construction]);

	m_pResourceDemand[EResources::Marketing_Advertising] = (((l_fTemp * (0.1f * 1349.f)) + ((REAL64)m_iPopulation * 8.f))*0.98f) 
...
bool GCountryData::FetchCountryData(const ENTITY_ID in_iCountryID)
{
   ...
   //Fetch the Covert Action Cells information
   {
      ...
      for(UINT32 i = 0 ; i < l_Table.RowCount() ; i++)
      {
         ...
         gassert(static_cast<ENTITY_ID>(m_iCountryID) == l_iOwnerID, L"Country ID and cell owner ID mismatch, " + GString(m_iCountryID) + L" vs " + GString(l_iOwnerID));
...
{
    gassert(in_fLifeExpectancy >= 0 && !_isnan(in_fLifeExpectancy), L"Invalid in_fLifeExpectancy " + GString(in_fLifeExpectancy));
...
{
    gassert(in_fMeanYearsSchooling >= 0 && !_isnan(in_fMeanYearsSchooling), L"Invalid in_fMeanYearsSchooling " + GString(in_fMeanYearsSchooling));
...
{
    gassert(in_fExpectedYearsSchooling >= 0 && !_isnan(in_fExpectedYearsSchooling), L"Invalid in_fExpectedYearsSchooling " + GString(in_fExpectedYearsSchooling));
...
bool GCountryData::OnSave(GIBuffer& io_Buffer)
{
   ...
   const bool l_bCompatibleSaveFormat = g_SP2Server->CompatibleSaveFormat();
   // GDZLOG(EDZLogLevel::Info1, L"compatibleSaveFormat = " + GString(l_bCompatibleSaveFormat));
...
bool GCountryData::OnLoad(GOBuffer& io_Buffer)
{
	...
	// Load the covert action cells
   {
      UINT32 l_iCount;
      io_Buffer >> l_iCount;
      GDZLOG(EDZLogLevel::Info1, NameAndIDForLog() + L" has " + GString(l_iCount) + L" covert cells");
      m_vCovertActionCells.resize(l_iCount);
      for(UINT32 i = 0;i < m_vCovertActionCells.size();i ++)
      {
         m_vCovertActionCells[i].Unserialize(io_Buffer);

         const ENTITY_ID l_iCellOwnerId = m_vCovertActionCells[i].OwnerID();
         if(l_iCellOwnerId != static_cast<ENTITY_ID>(m_iCountryID))
         {
             g_Joshua.Log(L"Cell " + GString(m_vCovertActionCells[i].ID()) + L" has inconsistent owner ID, cell has owner ID " + GString(l_iCellOwnerId) + L" but loading country ID " + GString(m_iCountryID) + L"; changing cell ownership", MSGTYPE_WARNING);
             m_vCovertActionCells[i].OwnerID(m_iCountryID);
         }
...
void GCountryData::InitHdiComponents()
{
    GDZLOG(EDZLogLevel::Entry, L"");
